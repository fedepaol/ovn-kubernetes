...
func newSharedGatewayOpenFlowManager(patchPort, exGWPatchPort string, gwBridge, exGWBridge *bridgeConfiguration) (*openflowManager, error) {
	...
	if ofm.exGWBridge {
		ofm.updateHostFlowCacheEntry("NORMAL", []string{fmt.Sprintf("table=0,priority=0,actions=%s\n", util.NormalAction)})
		ofm.updateHostFlowCacheEntry("DEFAULT", exGWBridgeDftFlows)
	}
...
func newSharedGateway(nodeName string, subnets []*net.IPNet, gwNextHops []net.IP, gwIntf, egressGWIntf string, nodeAnnotator kube.Annotator) (*gateway, error) {
	...
	fmt.Println("FEDE ex bridge", egBridge)
	// add masquerade subnet route to avoid zeroconf routes
	addMasqueradeRoute(gwBridge.bridgeName, gwNextHops)
	...
	exGWPatchPort := ""
	if egressGWIntf != "" {
		addMasqueradeRoute(egBridge.bridgeName, gwNextHops)
