...
func gatewayInitInternal(nodeName, gwIntf, egressGatewayIntf string, subnets []*net.IPNet, gwNextHops []net.IP, nodeAnnotator kube.Annotator) (
	*bridgeConfiguration, *bridgeConfiguration, error) {
	...
	egressGWBridge := &bridgeConfiguration{}
	if egressGatewayIntf != "" {
		egressGWBridge, err = bridgeForInterface(egressGatewayIntf, nodeName, types.PhysicalNetworkEgressGWName)
		if err != nil {
			return nil, nil, errors.Wrapf(err, "Bridge for interface failed for %s", gwIntf)
		}
	}
	...
	if config.Gateway.Mode == config.GatewayModeLocal {
		err = setupLocalNodeAccessBridge(nodeName, types.PhysicalNetworkName, subnets)
		if err != nil {
			return nil, nil, err
		}
	}
	...
	err = util.SetL3GatewayConfig(nodeAnnotator, &util.L3GatewayConfig{
		...
		EgressGWInterfaceID: egressGWBridge.interfaceID,
		EgressGWMACAddress:  egressGWBridge.macAddress,
		EgressGWIPAddresses: egressGWBridge.ips,
		NextHops:            gwNextHops,
