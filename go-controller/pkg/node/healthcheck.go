...
type openflowManager struct {
	...
	exGWGwBridge    string
	exGWPhysIntf    string
	exGWPatchIntf   string
	exGWOfportPatch string
	exGWOfportPhys  string
...
func (c *openflowManager) updateHostFlowCacheEntry(key string, flows []string) {
	c.exGWFlowMutex.Lock()
	defer c.exGWFlowMutex.Unlock()
	c.exGWFlowCache[key] = flows
}

func (c *openflowManager) deleteHostFlowsByKey(key string) {
	c.exGWFlowMutex.Lock()
	defer c.exGWFlowMutex.Unlock()
	delete(c.exGWFlowCache, key)
}
...
func (c *openflowManager) syncFlows() {
	...
	if c.exGWBridge {
		...
		_, stderr, err := util.ReplaceOFFlows(c.exGWGwBridge, flows)
		if err != nil {
...
func (c *openflowManager) Run(stopChan <-chan struct{}) {
	for {
		...
		case <-time.After(15 * time.Second):
			if err := checkPorts(c.patchIntf, c.ofportPatch, c.physIntf, c.ofportPhys); err != nil {
				klog.Errorf("checkports failed %v", err)
				continue
			}
			if c.exGWBridge {
				if err := checkPorts(c.exGWPatchIntf, c.exGWOfportPatch, c.exGWPhysIntf, c.exGWOfportPhys); err != nil {
					klog.Errorf("checkports failed %v", err)
					continue
				}
			}
...
func checkPorts(patchIntf, ofPortPatch, physIntf, ofPortPhys string) error {
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to get ofport of %s, stderr: %q, error: %v", patchIntf, stderr)
	}
	...
	if err != nil {
		return errors.Wrapf(err, "Failed to get ofport of %s, stderr: %q, error: %v", physIntf, stderr)
	}
